        ...
        <div className="space-y-8 pb-10">
            ...
            <div className="flex items-end justify-between">
                <div className="space-y-1">
                    ...
                    <p className="text-muted-foreground text-sm font-medium">Platform-wide overview and organization management.</p>
                </div>
                <div className="flex items-center gap-3">
                    <Button variant="outline" size="sm" className="h-9 bg-muted border-border text-xs font-semibold rounded-lg hover:border-primary/50 transition-colors">
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                {/* Deployment Log */}
                <Card className="lg:col-span-2 bg-card border-border rounded-xl overflow-hidden shadow-none">
                    <CardHeader className="flex flex-row items-center justify-between border-b border-border px-6 py-4 bg-foreground/5">
                        <CardTitle className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Recent Deployments</CardTitle>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="border-border hover:bg-transparent bg-background">
                                    <TableHead className="text-[10px] font-bold text-muted-foreground pl-6 h-10 uppercase tracking-wider">Account</TableHead>
                                    <TableHead className="text-[10px] font-bold text-muted-foreground h-10 uppercase tracking-wider">Plan</TableHead>
                                    <TableHead className="text-[10px] font-bold text-muted-foreground text-right h-10 uppercase tracking-wider">Seats</TableHead>
                                    <TableHead className="text-[10px] font-bold text-muted-foreground text-right pr-6 h-10 uppercase tracking-wider">Health</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {isLoading ? (
                                    Array(6).fill(0).map((_, i) => (
                                        <TableRow key={i} className="border-foreground/5">
                                            <TableCell className="pl-6 py-4"><div className="h-3 w-24 bg-muted animate-pulse rounded" /></TableCell>
                                            ...
                                            <TableCell className="pr-6 text-right"><div className="h-3 w-16 ml-auto bg-muted animate-pulse rounded" /></TableCell>
                                        </TableRow>
                                    ))
                                    ...
                                    <TableRow key={org.id} className="border-foreground/5 hover:bg-foreground/5 transition-colors">
                                        <TableCell className="pl-6 py-4">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[10px] text-muted-foreground font-medium">Created {new Date(org.created_at).toLocaleDateString()}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <Badge variant="outline" className="bg-muted border-border text-[10px] font-bold text-muted-foreground px-1.5 py-0 rounded lowercase">
                                                {org.plan}
                                            </Badge>
                                        </TableCell>
                                        <TableCell className="text-right text-xs text-muted-foreground font-mono font-bold">
                                            {org._count.users}
                                        </TableCell>
                ...
                <div className="space-y-6">
                    ...
                    <Card className="bg-card border-border rounded-xl overflow-hidden shadow-none border-t-2 border-t-primary/30">
                        <CardHeader className="pb-2">
                            <div className="flex items-center justify-between">
                                <CardTitle className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Operator Console</CardTitle>
...
function MetricCard({ title, value, icon: Icon, trend }: any) {
    return (
        <Card className="bg-card border-border rounded-xl shadow-none hover:border-border transition-colors group">
            <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                    <div className="h-10 w-10 border border-border rounded-lg flex items-center justify-center bg-muted group-hover:bg-primary/10 group-hover:border-primary/20 transition-all">
                ...
                <div>
                    <p className="text-[11px] text-muted-foreground uppercase font-black tracking-widest mb-1">{title}</p>
...
function HealthRow({ label, status, latency }: any) {
    return (
        <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
                <span className="text-[10px] font-mono font-bold text-muted-foreground">{latency}</span>
