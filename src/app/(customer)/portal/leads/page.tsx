        ...
        <div className="space-y-8">
            ...
            <div className="flex items-center justify-between">
                <div className="space-y-1">
                    ...
                    <p className="text-foreground/50">View all leads being nurtured for your campaigns</p>
                </div>
            </div>
            ...
            {summary.total > 0 && (
                <Card className="bg-foreground/[0.02] border-foreground/5">
                    <CardHeader>
                        <CardTitle className="text-base text-foreground">Lead Pipeline</CardTitle>
                    </CardHeader>
                    <CardContent>
            ...
            <Card className="bg-foreground/[0.02] border-foreground/5">
                <CardHeader className="flex flex-row items-center justify-between">
                    <CardTitle className="text-base text-foreground">Recent Leads</CardTitle>
                    <p className="text-sm text-foreground/40">Showing {leads.length} of {summary.total}</p>
                </CardHeader>
                <CardContent>
                    {leads.length > 0 ? (
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-foreground/5">
                                        <th className="text-left py-3 px-4 text-xs font-medium text-foreground/50">Name</th>
                                        <th className="text-left py-3 px-4 text-xs font-medium text-foreground/50">Company</th>
                                        <th className="text-left py-3 px-4 text-xs font-medium text-foreground/50">Icebreaker</th>
                                        <th className="text-left py-3 px-4 text-xs font-medium text-foreground/50">Status</th>
                                        <th className="text-left py-3 px-4 text-xs font-medium text-foreground/50">Added</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {leads.map((lead) => (
                                        <tr key={lead.id} className="border-b border-foreground/5 hover:bg-foreground/[0.02]">
                                            <td className="py-3 px-4">
                                                <div>
                                                    <p className="text-sm font-medium text-foreground">{lead.name}</p>
                                                    <p className="text-xs text-foreground/40">{lead.email}</p>
                                                </div>
                                            </td>
                                            <td className="py-3 px-4">
                                            ...
                                            <td className="py-3 px-4 text-sm text-foreground/40">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="flex items-baseline gap-2">
                            ...
                            {subtext && <span className="text-xs text-foreground/40">{subtext}</span>}
                        </div>
    ...
    return (
        <div className="flex flex-col items-center">
            <div className={`h-12 w-12 rounded-full ${color} flex items-center justify-center mb-2`}>
                <span className="text-sm font-bold text-foreground">{count > 999 ? `${Math.round(count / 1000)}k` : count}</span>
            </div>
            <p className="text-xs text-foreground/60">{label}</p>
            <p className="text-xs text-foreground/30">{percentage.toFixed(0)}%</p>
        </div>
    )
}
